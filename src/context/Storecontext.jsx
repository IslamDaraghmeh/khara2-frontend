...
const StorecontextProvider = (props) => {
    ...
    const addToCart = async (itemId) => {
        ...
        if (token) {
            await axios.post('https://suarez83.com/api/v1/cart/add', { itemId }, { headers: { token } });
        }
    };
    ...
    const removeCart = async (itemId) => {
        if (cartItem[itemId] > 0) {
            ...
            if (token) {
                ...
                await axios.delete("https://suarez83.com/api/v1/cart/delete", config);
            }
        }
    ...
    const getProductList = async () => {
        try {
            const response = await axios.get('https://suarez83.com/api/v1/basic/all');
            if (response.data) {
    ...
    const DeleteCartData = async (itemId) => {
        ...
        try {
            if (token) {
                await axios.delete('https://suarez83.com/api/v1/cart/deleteItem', { data: { itemId }, headers: { token } });
            }
        } catch (error) {
    ...
    const getCartData = async () => {
        try {
            const response = await axios.get('https://suarez83.com/api/v1/cart/get', { headers: { token } });
